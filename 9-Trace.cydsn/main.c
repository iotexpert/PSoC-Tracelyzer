...
{
    UART_UartPutString("Malloc Failed\n");
    RED_Write(0);
    CYASSERT(0);
}
...
void LED_Task(void *arg)
{
    ...
    UART_UartPutString("Entered BLINK Task\n");
	...
	while(1) {
        ...
        switch(c)
        {
            ...
            case '?':
                    ...
                    UART_UartPutString("h-print the free heapsize\n");
...
int main(void)
{
    ...
    result = xTaskCreate(
     ...
     UART_UartPutString("LED Task Started\n");
   
    sprintf(buff,"Before UART Heap Free = %d\n",xPortGetFreeHeapSize());
    UART_UartPutString(buff);
    ...
    result = xTaskCreate(
        ...
        400,             /* Task stack, allocated from heap */
        0,               /* No param passed to task function */
        1,               /* Low priority */
        0 );             /* Not using the task handle */
    ...
    {   
        RED_Write(0);
        sprintf(buff,"UART Task Failed heapFree = %d\n",xPortGetFreeHeapSize());
        UART_UartPutString(buff);
        CYASSERT(result == pdPASS);   
    }
    UART_UartPutString("UART Task Started\n");
    //GREEN_Write(0);  
    sprintf(buff,"After UART/Before I2C Heap Free = %d\n",xPortGetFreeHeapSize());
    UART_UartPutString(buff);
    ...
    sprintf(buff,"Heap Free after I2C = %d\n",xPortGetFreeHeapSize());
    UART_UartPutString(buff);
   
    if(result != pdPASS)
    {   
        sprintf(buff,"I2C Master Task Failed heapFree = %d\n",xPortGetFreeHeapSize());
        UART_UartPutString(buff);
        CYASSERT(result == pdPASS);   
    }
   
    UART_UartPutString("I2C Master Task Started\n");
    
     sprintf(buff,"After I2C = %d\n",xPortGetFreeHeapSize());
    UART_UartPutString(buff);
